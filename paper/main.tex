...
\cite{pagnoncelli2023} building on this this evidence, has argued  for replacing 
asset-class limits altogether with a single portfolio-level risk 
metric. 

ZZZZZZZZZZZZZZZZZZZZZZZZZZZ

NOTE  I  INSERTED THIS paragrph ---BELOW  (new paper) needs to be added to REF----------

ZZZZZZZZZZZZZZZZZZZZZZ

A recent contribution by Perchet, Hacini, Heckel, and Somefun (2025) shares
...
at time $T$ is determined ex ante and depends only on $T$, not on 
...
We assume a deterministic salary path consistent with the OECD methodology used in \emph{Pensions at a Glance}: constant real annual growth of 1.25\% throughout the working career \cite{oecd2025}.  For ease of interpretation, we present the salary path as an index normalized to 100 at the age of labor-market entry ($\Tstart=25$  years). Under the baseline assumption it reaches approximately 164 by the statutory retirement age ($\TB=65$  years), as shown in Figure~\ref{fig:salary-path}.


ZZZZZZZZZZZZZZZZZZZZ

NOTE---- YO SACARIA FIG  2--  MUY FOME--NO AGREGA MUCHo; en ese caso hay que reescribir, o sea, cambiar un poco el parrafo en cuestion, arriba.  TO BE DISCUSSED-- y dejar algo mas   o menos asi


\textbf{Salary evolution.} We assume a deterministic salary path 
consistent with the OECD methodology used in \textit{Pensions at a 
Glance}: constant real annual growth of $1.25\%$ throughout the working 
career~\cite{OECD2025}. Starting from an index value of $100$ at 
labor-market entry ($T_{\text{start}} = 25$ years), the salary reaches 
approximately $164$ by the statutory retirement age ($T_B = 65$ years).
...
The working horizon runs from $\Tstart=25$ year to $\TB=65$ years: the OECD convention sets the entry at age 22, but the average age of new affiliates in Chile is 28, thus we use the midpoint \cite{oecd2025,sp2024ficha}; $\TB=65$ years is the current statutory retirement age for men \cite{dl3500}. 
...
Given $\Kstar$, $\Rstar$ is the constant annualized real return that, applied to the stream of monthly contributions over the working career, exactly accumulates $\Kstar$ at retirement. It is determined by solving:

\begin{equation}
K^* = \lambda \sum_{t=1}^{N} S_t (1 + R^*)^{N-t},
\end{equation}
where $N = 12 \times (T_B - T_{\text{start}}) = 480$ months, $S_t$ is 
...
The equation is solved numerically with an iterative procedure.
...
Modeling each worker's individual contribution pattern explicitly 
is beyond the scope of this study. Instead, we adopt a transparent 
approximation: rather than alternating between full contributions 
...
\begin{equation}
K^* = \beta\lambda \sum_{t=1}^{N} S_t (1 + R^*)^{N-t}.
\end{equation}
...
\paragraph{Candidate glidepath assessment.}
 ...
 review-- hice un cambio pequenno en este parrafo--arriba
...
\begin{figure}[!htbp]
  ...
  \caption{Heatmap of success probability $\Psi$ over the $(A,\TA)$ grid for $\Rstar=\text5.5\%$. The 50\% contour separates successful from unsuccessful CVaR glidepaths.}
  \label{fig:heatmap}
\end{figure}
...
\begin{figure}[!htbp]
  ...
  \caption{Percentage of successful glidepaths  ($\Psi>50\%$) and average success rate across all glidepath, as a function of contribution density.}
...
$\Psi > 50\%$ success threshold, and the average $\Psi$ across all (sucessful and unsucessful) glidepaths. 
...
infeasible---no portfolio investment strategy can compensate for 
...
Between these boundaries---roughly $59\%$ to $67\%$---lies the region where 
glidepath design matters most. Within this window, the share of successful 
...
($58.3\%$) falls below the lower bound of the feasible region---meaning that 
under the baseline assumptions, no successful glidepath exists for the average 
Chilean male worker. The observed female density ($49.6\%$) lies well outside 
it. At current Chilean densities, the glidepath design problem is simply 
infeasible for the representative worker.
...
materially above $58\%$, other levers---such as the contribution rate, the 
retirement age, or the replacement rate target---must be reconsidered, since 
no glidepath can deliver the required return ($R^*$) under the baseline assumptions at those 
density levels.
...
(60 vs.\ 65 years); (ii) longer post-retirement life expectancy (90 vs.\ 86 
years); and (iii) lower contribution density ($49.6\%$ vs.\ $58.3\%$) 
\cite{dl3500,sp2023mortality,sp2024ficha}. Consequently, the gender-specific 
required return differ substantially: $R^* = 5.3\%$ for men and 
...
NOTE: I am tempted to eliminate this section (or alternatively) shortering quite a bit.  It is very well written, but my concern is that it might be too much, a bit distracting.  And also it seems that the tendency/ push is to eliminate the retiro progamado.  Open  to discuss//  on the other hand, for arXiv (and later a journal) unless the reader is from Chile this might not be relevant 
...
\subsection{Choice of the Eligble Assets and Implications}
...
NOTE---this  is   tentative --- THIS IS TO BE DISCUSSED--Just put them as bullet points  now ///   can write properly later if everybody is Ok with this 
...
canonical (Markowitz) mean--variance paradigm. Rather than maximizing return 
...
Adequate pensions for women require structural refoems such as higher contribution rates, adjusted 
