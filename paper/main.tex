...
At present, early 2026, Vanguard is the largest  player in this space: with \$1.8 trillion in AUM, it accounts for 37\% of this segment. Fidelity is a distant second with \$693 billion. All in all, the five biggest players account for roughly 80\% of this market \cite{roy2026}.
...
ambiguous support for this convention.  Moreover, they rely  on utility-theoretic 
...
portfolio-level metric such as the CVaR, rather than \emph{indirectly}, 
...
Beyond the simple rules, a number of alternative glidepath designs have been proposed---fixed-mix 60/40 portfolios, rising-equity paths in retirement \cite{pfau2013}, and ``U-shaped'' trajectories \cite{estrada2019a,estrada2019b}---with mixed empirical support. None of these proposals, however, departs from the core mechanism of conventional TDFs: risk is controlled indirectly, through asset-class limits or bands.  Two examples from the industry support this view. T. Rowe Price, for instance, a company that offers investment solutions to institutional and retail investors, carried out a study that explored the merits of different TDF designs based on Monte Carlo simulations.  All the alternatives, nevertheless, were based on somewhat ad-hoc stocks-bonds combinations but with no reference to any portfolio-based risk metric \cite{trowe2016}. NISA, a company active in the investment consulting space, in a similar study, considered several stocks-and-bonds combinations which were described only qualitatively in terms of risk (that is, with no reference to any specific metric or goal) \cite{nisa2016}. 
...
patterns.   At present, the Chilean pension system is structured based on five  funds (known as A, B, C, D and E); Fund A is supposed to be the most aggressive and E the most conservative. However, Fund E  has outperformed 
...
inverted relative to the regulator's original intent. In Mexico, by contrast, where asset-class limits are combined with a portfolio-level VaR 
constraint, the funds rank as intended. A subsequent study, 
 building on this evidence, has argued  for replacing 
asset-class limits altogether with a single portfolio-level risk 
metric  \cite{pagnoncelli2023}. 



A separate strand of the literature has advocated for adaptive 
TDFs, in which asset allocations at time $T$ depend not only on $T$ 
...
Several authors have gone beyond describing the asset composition of TDFs and have computed outcome-based performance metrics. For instance, Basu et al. \cite{basu2011} evaluated lifecycle strategies using terminal wealth distributions, shortfall probabilities, and Expected Tail Loss; Menoncin and Vigna \cite{menoncin2017} derived ruin probabilities and efficient frontiers for DC pension schemes; and Forsyth et al. \cite{forsyth2017b} assessed TDF performance through expected terminal wealth and probability of shortfall at retirement. Yoon \cite{yoon2010} went  a step further by deriving the asset allocation from a pre-specified risk budget. However, none of these studies explicitly linked the asset composition of the TDF to the outcome metrics in the design stage.  In other words, the metrics were  used to evaluate or compare strategies \emph{ex post}, rather than to construct the glidepath itself.



A recent study by \citet{perchet2025} shares
our motivation.  These authors manage risk through a portfolio-level metric,
...
A comprehensive and up-to-date review of the TDF literature, with many references to the Chilean pension system,  is provided in \cite{suarez2025}.
...
objective into a risk budget (described  through a suitable metric) that gives the fund a high probability 
of meeting it.   In short, currently there is no universally accepted method to construct a risk glidepath based on a specific, and measurable, objective. It is this gap that the present study aims to fill.
...
The objective is to determine a CVaR-limit glidepath---a  declining CVaR limit trajectory over the TDF's life---calibrated to deliver enough capital to finance an adequate annuity at the fund's closing date.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=0.13cm,y=0.42cm,>=Latex]
    ...
    \node[left] at (0,3) {$B$};
  \end{tikzpicture}
  \caption{Schematic CVaR-limit (glidepath) curve   {$L(t)$}. The TDF is constrained to an  initial CVaR  limit $A$ until $\TA$, then it linearly reduces the limit to $B$ by retirement age $\TB$.}
...
(assuming both $T_B$ and $T_{\text{start}}$ are expressed in years), is
...
Second, we assume that the TDF can invest in a universe of $N$ assets, each with a sufficiently long return history---covering $M$ months of past monthly returns---to support simulations of future returns scenarios.
...
In essence, the approach to solving such a problem consists of assuming a glidepath structure 
and then estimating the probability that it delivers the required 
return $R^*$.  If such a probability is high enough, the glidepath is deemed acceptable.


Thus, for a given CVaR glidepath $L$, defined by a given set of values $A$, $B$, $\Tstart$, $\TA$ and $\TB$, we need to estimate the probability that this glidepath can deliver the required return $\Rstar$.
...
\end{equation}
These scalar returns are then sorted from lowest to highest, and the CVaR is estimated as the average of the worst ten percent of them, since we work with a 90\% confidence level.  The CVaR, strictly speaking, is a negative number (it describes losses); however, for convenience, it is often customary to express it as a positive number.  We adopt this choice in what follows. 

\paragraph{Feasible asset-allocations array.}
Using a Monte Carlo sampling algorithm based on the Hit-and-Run method (see \hyperref[app:hit-and-run]{Appendix}; the algorithm follows \citet{smith1984}), we construct a feasible asset allocation array  $\mathbf{\Omega}$  of dimension $I \times Q$, where $I$ denotes the number of feasible portfolios to be constructed. Each entry $(i,t)$ of this array corresponds to an $N$-dimensional vector $\Omega$ that satisfies two conditions: (a) its CVaR limit is below $L(t)$, the limit specified by the glidepath for month $t$; and (b) the set of return vectors used to compute its CVaR corresponds to column $t$ of the return array $\mathcal{R}$. That is, the set of return vectors employed to calculate the CVaR for month $t$ is $\{R_{s,t}\}_{s=1}^{S}$.

\paragraph{Returns scenarios and feasible portfolios.}
We assume that the TDF manager rebalances the portfolio monthly. As mentioned before, a returns scenario $s$   is defined as a sequence of $Q$ return vectors of dimension $N$, that is, $\{R_{s,1},\ldots,R_{s,Q}\}$. From the returns array we have $S$ such scenarios, where each row $s$ of that array defines one such scenario.

We define a feasible portfolio $P_i$, for $i \in \{1,\ldots,I\}$, as a sequence of $Q$ feasible asset allocation vectors, that is, $P_i=\{\Omega_{i,1},\ldots,\Omega_{i,Q}\}$. To construct $I$ feasible portfolios from the asset-allocation array, rather than taking its rows directly as generated by the Hit-and-Run sampler---which could introduce spurious serial correlation between consecutive monthly asset allocations, since the Hit-and-Run algorithm generates samples sequentially and adjacent draws are not fully independent---we first apply an independent random permutation scheme within each column of this array. In short, feasible portfolio $i$ is obtained by taking the $i$th row of the  permuted array.

This construction sets the stage for modeling the behavior of a portfolio manager who does not follow any  intertemporal strategic rule, but rather  chooses randomly, every month, an asset allocation from the set of feasible asset allocations (namely, asset allocations that satisfy the CVaR limit for that specific month).
...
\end{equation}
This figure of merit can be interpreted as the probability that a hypothetical portfolio manager, who randomly selects a feasible portfolio from the set of all feasible portfolios, will succeed in delivering the required return $\Rstar$. This is a diagnostic interpretation of the feasible set, not the estimated success probability of an average professional manager. The lower the value of $\Psi$, the less desirable the glidepath is, from a regulatory standpoint.
...
For readability, we drop the ``UF'' qualifier throughout.
 Because the UF captures  inflation, every figure here should be read as a real  (inflation-adjusted) figure. 
...
We take life expectancy at retirement to be  88 years, which according to 
...
of 20 UF per month  (roughly equivalent to USD~900, and 
...
\end{equation}
Under the baseline density $\beta=60\%$---chosen to be slightly above the observed male average of 58\%, so that the design problem is feasible while remaining realistic---and the other parameters defined previously, the solution is r* = 0.004472, which yields 
...
statistics for each asset class; CVaR is computed at the 90\% confidence level.  Returns are expressed in Chilean inflation-adjusted (UF) pesos; no currency hedging is involved.

\begin{table}[H]
  \centering
  \small
  \caption{List of eligible assets: key statistics based on monthly real returns (inflation-adjusted Chilean pesos); time period covered, April 2007-December 2025.}
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X l r r r@{}}
    ...
    \midrule
    LVA-Chile Certificates of Deposit (CD) & LKXIP & 0.01\% & 0.37\% & 0.62\% \\
    ...
    MSCI World (Global equities) & NDDUWI & 0.63\% & 4.41\% & 7.78\% \\
    MSCI Emerging Markets (EM equities) & NDUEEGF & 0.39\% & 4.92\% & 8.56\% \\
...
fixed-income component, we use the Riskamerica Corporate Bonds
...
Unless otherwise noted, the results use the Gaussian-copula return model,
the nine-asset universe, and the constant-fraction approximation to
contribution density described in Section 5.1. The reported thresholds
apply to the 210 candidate glidepaths considered here and the
$\Psi>50\%$ success criterion.
...
Finally, two observations regarding the robustness of the simulation results are worth mentioning.  First, the computation of $\Psi$ for each glidepath was based on the percentage  of the $I \times S$ (feasible portfolios $\times$ returns scenarios) combinations that met the minimum return condition (R* = 5.5\%).  Alternatively, one could have defined success in terms of the minimum wealth required to achieve the desired pension, that is, 3,800 in this case.  (Notice, from the previous section, that the target return, 5.5\%,  was derived from the 3,800 figure.)  Computation of $\Psi$ based on this criterion (accumulated wealth) was consistent with the results obtained based on R* = 5.5\%.  In fact, averaging the discrepancies between the values of $\Psi$ computed with either approach over the 210 glidepaths analyzed yielded an average difference of less than 2\%; in essence, simulation noise rather than a conceptual difference with  practical implications.  Briefly, both approaches to assess glidepath suitability yielded consistent results.

Second, for computational efficiency the Gaussian copula-based engine was used only once to generate an $S \times Q$ array of simulated returns vectors (denoted as $\mathcal{R}=\{R_{s,t}\}$ as previously  indicated).  That is, the same array of returns vectors was employed to estimate the CVaRs (feasible portfolios) and returns along each returns scenario.  An \textit{ex post} (more computationally intense) analysis, based on two different arrays of returns (one to estimate the CVaRs, the second to estimate returns), both generated with the same Gaussian copula engine, yielded almost identical results.
...
To isolate the role of the transition age $\TA$, we consider a restricted version of the problem in which $A$ and $B$ are fixed. Specifically, we set $A=10\%$---essentially leaving the risk unconstrained initially, giving managers maximum flexibility in the early phase---and $B=3\%$ as before. The only question is then: when should the transition age begin?
...
contribution rates, or strengthening solidarity mechanisms (e.g., state-funded pension complements).
...
The case of women is difficult because these three parameters all move in the 
...
Finally, the CVaR estimates and the Hit-and-Run samples are finite-simulation objects. Each monthly CVaR uses the worst 10\% of $S=10{,}000$ simulated return outcomes, and the feasible asset allocations are sampled with a burn-in of $b_{\mathrm{burn}}=20$. In practice, the impact of this short burn-in is mitigated by the scale of the retained sample: for each month we keep $I=10{,}000$ post-burn-in draws, and the cross-month permutations described in the Appendix    further dissociate the trajectories used to evaluate $\Psi$ from the sequential ordering of any single Hit-and-Run chain. These implementation choices are therefore unlikely to drive the broader patterns reported above, which are dominated by the shape of the glidepath  and contribution density.
...
to the Appendix, it is worth emphasizing that the algorithm is 
...
requirement (e.g., measured via the Herfindahl-Hirschman index), a maximum 
...
CVaR glidepath. (It is implicitly assumed that the glidepath, presumably specified by the regulator, is consistent with a reasonably  high probability of achieving the desired return.)   Then,  the  portfolio manager  selects from within that 
set  an asset allocation. This formulation places less reliance on the manager's ability 
to identify an optimal portfolio, since he/she is making the selection from a set of choices that by design   carry a high probability of meeting the 
return target.
...
begins reducing risk---is a critical design variable; glidepaths that de-risk too early consistently fail to deliver the required return ($R^*$), 
...
\section*{APPENDIX\quad Uniform Sampling over the Feasible Set via the Hit-and-Run Algorithm}
