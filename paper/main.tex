...
To isolate the role of the transition age $\TA$, we consider a restricted version of the problem in which $A$ and $B$ are fixed at policy-relevant values. Specifically, we set $A=10\%$---effectively non-binding relative to the asset universe, since it exceeds the highest single-asset CVaR in Table~\ref{tab:asset-universe}---and $B=3\%$ as before. This gives managers maximum flexibility in the early phase while preserving a conservative terminal CVaR limit. The only question is then: at what age can risk-taking begin to decline without losing sufficiency?
...
Given this interpretation, the central design problem is one of constrained optimality: among CVaR-limit glidepaths that satisfy the sufficiency criterion, how little cumulative risk can the regulator allow? The exercise operationalizes this question over an interpretable policy class of piecewise-linear CVaR curves. This class is deliberately simple, but not narrow in a regulatory sense: many relevant decreasing risk paths can be represented directly in this form, and more irregular decreasing paths can be conservatively approximated or enveloped by a piecewise-linear CVaR ceiling.

Within this class, and fixing the terminal CVaR limit at $B=3\%$---a reasonably conservative level for portfolios close to retirement---the full-pool optimum $(A,B,\TA)=(6\%,3\%,58)$ is the minimum-$\Gamma$ successful point of the policy grid considered here. It should not be interpreted as a globally optimal glidepath over all possible dynamic risk policies, but it is a meaningful optimum within the regulatory design problem studied in the paper and under the simulation conditions of the exercise, including the terminal CVaR limit of 3\%, the selected asset indices, return-generation engine, and Monte Carlo implementation.

Beyond the exact optimum, the analysis shows that direct control of portfolio-level CVaR can generate interpretable glidepath rules. For example, instead of prescribing age-dependent equity allocations, the regulator can specify a path of admissible downside risk and ask whether it is compatible with a target replacement objective. Under the baseline calibration, the relevant trade-off is between the amount of risk allowed early in life and the age at which de-risking begins: lower early-life risk requires delaying the transition, while higher early-life risk allows the transition to start earlier. In particular, the results can answer the following question: what is the earliest age at which risk can begin to decline while still satisfying the minimum-sufficiency criterion? In the filtered exercise, this question is evaluated under a high early-life CVaR limit, fixed at $A=10\%$, and the same terminal CVaR limit used throughout the analysis, $B=3\%$. Under those conditions, the answer is approximately age 45, according to the filtered specification $(A,B,\TA)=(10\%,3\%,45)$, which represents the first transition age that crosses the sufficiency threshold. Values of $\TA$ below 45 fail even under this generous early-risk allowance. Thus, the method is useful both for constructing policy-relevant CVaR glidepaths and for addressing concrete policy questions about how early risk can be reduced without losing sufficiency.
