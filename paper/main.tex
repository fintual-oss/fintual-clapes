...
\begin{abstract}
We propose a framework for designing target-date fund (TDF) glidepaths around a declining portfolio-level Conditional Value-at-Risk (CVaR) trajectory calibrated to a minimum-sufficiency criterion: a target replacement rate at retirement. Conventional TDFs control risk indirectly through age-dependent asset-class limits; we instead impose a regulator-specified portfolio CVaR ceiling while leaving the asset mix unconstrained. For each piecewise-linear CVaR-limit glidepath, we estimate the success probability as the fraction of portfolio--scenario combinations in which feasible portfolio trajectories reach the required real return. Feasible monthly allocations are sampled with the Hit-and-Run algorithm, and future returns are generated with a Gaussian-copula engine. We apply the framework to Chile's 2025 pension reform, using nine Chilean and global asset classes from April 2007 to December 2025, a 40-year accumulation horizon, and OECD wage assumptions. Across 210 candidate glidepaths, the results show that sufficient early-life risk-taking is important for pension sufficiency: under the baseline calibration, glidepaths that begin reducing risk before the worker's mid-forties fail to reach the target, even when the initial CVaR limit is high. They also show that contribution density is a binding constraint that portfolio design cannot overcome. At or below roughly 58\%, no candidate glidepath clears the 50\% success threshold, placing the female case under current densities outside the feasible region. The framework is general and applies to any TDF designed around an explicit return objective.
\end{abstract}
...
\subsection{Core Interpretation and Policy Implications}

The policy interpretation of the results proceeds in layers. We first clarify what the success statistic measures, then discuss what the glidepath results imply for regulatory design, and finally identify the economic constraints that can make the target infeasible regardless of the portfolio rule.
...
Given this interpretation, the qualitative glidepath results are more informative than the exact optimum, and they are the main design output of the exercise. The analysis shows that direct control of portfolio-level CVaR can generate interpretable glidepath rules. Instead of prescribing ad hoc equity shares, the regulator can specify a path of admissible downside risk and ask whether it is compatible with a target replacement objective. Under the baseline Chilean calibration, the robust pattern is a risk-timing trade-off. Lower early-life risk requires a later transition age. Higher early-life risk permits the transition to begin earlier. De-risking substantially before the mid-forties makes it difficult to reach the target return. The precise identity of the minimum-$\Gamma$ successful curve depends on the grid, the fixed terminal CVaR limit $B=3\%$, the candidate values of $A$, the assumed return process, and the finite-sample simulation. Consequently, $(A,B,\TA)=(6\%,3\%,58)$ should not be interpreted as a globally optimal glidepath, but only as the minimum-$\Gamma$ successful point of the policy grid considered here. Similarly, the filtered specification $(A,B,\TA)=(10\%,3\%,45)$ should be read as a policy benchmark. If the regulator allows a high early-life CVaR limit and requires a 3\% terminal limit, the transition can begin around mid-career, whereas materially earlier de-risking is not supported by the baseline exercise. Thus, the method is useful both for constructing policy-relevant CVaR glidepaths when the problem is feasible and for diagnosing cases in which contribution, retirement-age, or solidarity instruments must do the work.

This interpretation also affects how the 50\% threshold should be read. In the baseline grid, several candidate curves cross the 50\% success threshold by only a few basis points: the full-pool optimum, for example, has $\Psi=50.02\%$, and the filtered specification first crosses the threshold at $\Psi=50.13\%$. The point of the following calculation is not to claim a precise confidence interval, but to put the scale of these differences in perspective. If the $I \times S=10^8$ portfolio--scenario outcomes were independent Bernoulli trials with success probability close to 50\%, the standard error of $\Psi$ would be
\[
\sqrt{\frac{0.5(1-0.5)}{10^8}} \approx 0.00005 = 0.005\%.
\]
This calculation treats the $10^8$ indicators as fully independent observations. In the actual simulation this assumption is too strong: many outcomes are generated from common return scenarios, and the feasible allocations are sampled sequentially by the Hit-and-Run Markov Chain Monte Carlo (MCMC) sampler described in Appendix~\ref{app:hit-and-run}. Hence the effective amount of independent information is smaller than $10^8$, so the calculation should be read only as a lower-bound benchmark. For the present exercise, the implication is simple: values such as 50.02\% and 50.13\% should be read as essentially at the feasibility boundary. From a regulatory perspective, a glidepath that barely clears 50\% should not be viewed as materially different from one that barely fails.

The main economic constraint behind these feasibility results is contribution density. It is not merely another input parameter. It is the dominant feasibility parameter in the Chilean application. The density exercise shows a narrow transition region: below roughly 58\%, no glidepath in the grid succeeds, while above roughly 68\%, virtually all do. This sharp transition is economically intuitive. Lower density reduces contributions, which raises $\Rstar$ nonlinearly because the worker has less capital to compound over the same horizon. But the way we model density is intentionally simple: a worker contributes a constant fraction $\beta$ of the statutory contribution every month. This preserves total career contributions but suppresses timing risk. In reality, missed contributions early in life are more damaging than missed contributions near retirement, because they forgo decades of compounding. Conversely, contribution gaps clustered late in life have a different effect on $\Kstar$ and on realized wealth. A richer model would simulate employment histories, unemployment spells, informality, maternity-related gaps, and wage scarring jointly. The qualitative conclusion should therefore be read conditionally but clearly: under the assumptions of the exercise, sufficiently low contribution density can make the glidepath problem infeasible. The exact 58\%--68\% band should be read as approximate, since it depends on the smoothed density assumption and on the inputs used to compute $\Rstar$.

The gender results sharpen the same point. They should be understood as a diagnosis of structural constraints rather than as a recommendation to solve gender inequality through portfolio risk. The female case is difficult because several parameters move in the same direction: lower density, earlier retirement, and longer life expectancy. The resulting $\Rstar$ is so high that the investment problem becomes infeasible within the admissible risk range considered here. This is not a failure of the CVaR-glidepath methodology. The results above show that taking sufficient risk early in the life cycle is valuable, and that de-risking too early can be costly. In the female case, however, a more aggressive glidepath by itself is not enough to offset the structural gap. The relevant complementary instruments are therefore parametric and institutional---contribution incentives, retirement age, fiscal transfers, childcare and labor-market policies, annuity design, or solidarity mechanisms.

\subsection{Some Considerations Regarding Potential Statistical Biases}

The numerical results are conditional on a small set of modeling choices. The policy grid is intentionally simple: piecewise-linear CVaR curves vary $A$ and $\TA$ while the terminal limit $B$ is held fixed. We hold some parameters fixed both for simplicity and because the purpose is not to overfit a flexible curve to simulation noise. This keeps the exercise interpretable, but it also means that the minimum-$\Gamma$ curve is grid-dependent.

The CVaR confidence level is also fixed. Throughout the analysis we use CVaR at the 90\% confidence level, and we do not search over alternative tail levels to improve the results. A regulator could reasonably prefer a more conservative level, such as 95\% or 99\%, which could move the feasible frontier. For the present exercise, the relevant point is that the risk metric is applied consistently and not selected ex post.
